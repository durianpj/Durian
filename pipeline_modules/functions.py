...
# 함수 정의(def) 모음
# ══════════════════════════════════════════════════════════════════════════════
# 여기에는 '함수 정의'만 둔다. 이 함수들을 호출해서 단계 작업을 수행하는 코드는
# preprocess.py / convert.py / indexing.py 에 있고,
# 설정·상수는 config.py, 예외는 errors.py, 전체 흐름은 루트의 pipeline.py 에 있다.
import os
import shutil
import zipfile
...
from pipeline_modules.config import (
    # 1단계 전처리 검증용
    TODAY,
    MIN_AGE, MAX_AGE, MIN_SALARY, MAX_SALARY, MIN_OVERTIME, MAX_OVERTIME,
    MIN_UNUSED_LEAVE, MAX_UNUSED_LEAVE, MIN_GPA, MAX_GPA, MIN_SCORE, MAX_SCORE,
    MIN_TOEIC, MAX_TOEIC,
    DEPARTMENTS, DEPT_TEAM_MAP, DEPT_LEVEL_MAP, GRADE_LEVEL_MAP,
    POSITIONS, PERF_GRADES, INSURANCE_VALUES, SUBSIDY_VALUES, PERF_YEARS,
    # 2단계 변환용
    MISSING_VALUES, EMBEDDING_FIELDS,
    # 3단계 인덱싱/변경감지용
    EMBEDDING_DIM, KNN_ENGINE, KNN_METHOD, KNN_SPACE_TYPE, META_COMPARE_FIELDS,
    USER_DICT_FILE, LAST_USER_DICT_FILE,
    # 연결·모델·경로
    OPENSEARCH_HOME, OPENSEARCH_HOST, OPENSEARCH_PORT,
    OPENSEARCH_USER, OPENSEARCH_PASSWORD, OPENSEARCH_USE_SSL, OPENSEARCH_VERIFY_CERTS,
    EMBEDDING_MODEL, ERROR_LOG_PATH,
)
from pipeline_modules.errors import PipelineError, PipelineStop


# ── 실행 준비: OpenSearch 연결 · 임베딩 모델 로딩 ──────────────────────────────
def connect_opensearch():
    # OpenSearch에 연결하고 실제 통신이 되는지(client.info) 확인한다.
    print('\n========== OpenSearch 연결 ==========')
    client = OpenSearch(
    ...
    try:
        ...
        raise PipelineError(
            f'OpenSearch 연결 실패: {error}\n'
            f'OpenSearch가 실행 중인지 확인해주세요. ({OPENSEARCH_HOST}:{OPENSEARCH_PORT})'
        )
    return client
...
def load_embedding_model():
    ...
    try:
        ...
        raise PipelineError(
            f'임베딩 모델 로딩 실패: {error}\n'
            f'모델명({EMBEDDING_MODEL})과 네트워크 연결을 확인해주세요.'
        )


# ── 1단계 검증 함수들이 공유하는 전역 상태 (step1에서 fn.xxx 로 초기화/조회) ──
_errors    = []   # 에러 로그 모음
...
# ── 1단계 검증 실행기: 모든 validate_*를 한 df에 돌리고 결과를 반환 ─────────────
def run_validations(df):
    # 한 DataFrame에 모든 검증을 수행하고 (에러목록, 제거할 행번호 집합)을 반환한다.
    # 검증 함수들이 공유하는 이 모듈의 전역 상태(_errors/drop_rows/valid_rrn/valid_hire)를
    # 여기서 초기화·수집하므로, 호출하는 쪽(preprocess.py)은 전역을 직접 만질 필요가 없다.
...
def ensure_nori_plugin(client):
    ...
    raise PipelineStop('OpenSearch를 재시작한 후 다시 실행해 주세요.')


# ── 3단계: 텍스트 파싱 · 기존문서 조회 · 변경감지 함수 ─────────────────────
