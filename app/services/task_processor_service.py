...
def build_unknown_org_answer(unknown_orgs: list[str]) -> str:
    # 검색어 중 인식할 수 없는 조직명(unknown_orgs) 리스트가 비어 있다면, 
    # 기본 '검색 결과 없음' 메시지를 반환
    if not unknown_orgs:
        return NO_SEARCH_RESULT_MESSAGE

    # 리스트의 첫 번째 미등록 조직명을 대상으로 분석을 진행
    unknown_org = unknown_orgs[0]
    # 비교를 용이하게 하기 위해 공백이나 특수문자 등을 제거하여 텍스트를 압축/정렬 (예: "마케팅 팀" -> "마케팅팀")
    compact_unknown_org = compact_text(unknown_org)

    # 정의된 전체 부서(DEPARTMENTS) 목록을 순회하며 매칭되는 부서가 있는지 확인
    for department in DEPARTMENTS:
        # 부서명도 정규화한 뒤, 접미사 "부"를 제거하여 핵심 키워드만 추출 (예: "마케팅부" -> "마케팅")
        department_keyword = compact_text(department).removesuffix("부")

        # 부서 키워드가 유효하고, 유저가 입력한 미등록 조직명 안에 해당 부서 키워드가 포함되어 있다면 
        # (예: 유저가 "마케팅기획팀"을 입력했고, "마케팅" 키워드가 포함된 경우)
        if department_keyword and department_keyword in compact_unknown_org:
            # TEAM_TO_DEPARTMENT 맵핑 테이블에서 해당 부서(department)에 소속된 실제 팀 목록을 추출
            teams = [
                team
            ...
            # 해당 부서에 등록된 하위 팀들이 존재한다면, 안내 문구와 함께 올바른 팀 목록을 추천
            if teams:
                return (
            ...
            # 부서 키워드는 매칭되었으나 하위 팀 목록이 없는 경우의 기본 예외 문구
            return f"{unknown_org}은(는) 등록된 팀이 아닙니다."

    # 부서 목록을 모두 돌았음에도 키워드가 일치하는 부서를 찾지 못한 경우, 아예 알 수 없는 조직명으로 판단
...
def is_basic_profile_question(question: str) -> bool:
    # 질문 텍스트의 공백이나 특수문자를 제거하여 정규화 (예: "인사 프로필 조회" -> "인사프로필조회")
    compact_question = compact_text(question)
    
    # 정규화된 질문 안에 아래 정의된 기본 인사 프로필 관련 핵심 키워드가 하나라도 포함되어 있는지 검사
    return any(
...
def build_denied_message(denied_fields: list[str]) -> str:
    # 권한이 거부된 필드(denied_fields) 리스트가 비어 있다면 빈 문자열 반환
    if not denied_fields:
        return ""

    # 거부된 영어 필드명을 FIELD_RULES 맵핑 테이블을 참조하여 가독성 좋은 한글 라벨로 변환
    # (예: "salary" -> "연봉", 만약 맵핑 데이터가 없으면 원래 필드명 그대로 사용)
    labels = [
        FIELD_RULES.get(field, {}).get("label", field)
        for field in denied_fields
    ]

    # 변환된 한글 라벨들을 쉼표(,)로 연결하여 권한 거부 안내 메시지 완성 후 반환
...
def remove_self_placeholder_filters(filters: list[dict], is_self: bool) -> list[dict]:
    # 본인(자기 자신)에 대한 조회 요청이 아니라면, 필터 리스트를 가공하지 않고 그대로 반환
    if not is_self:
        return filters

    # 본인 조회 요청인 경우, '본인용 플레이스홀더(예: 자기 자신을 가리키는 더미 값)' 필터를 필터링함
    # 즉, 플레이스홀더 조건이 아닌 일반 필터 아이템들만 남겨서 새 리스트로 반환 (리스트 컴프리헨션)
    return [
        item
        for item in filters
...
def get_sort_value_from_hits(employee_hits: list[dict], field_key: str):
    # 검색된 사원 문서 리스트(employee_hits)를 순회하며 정렬에 사용할 유효한 값을 탐색
    for hit in employee_hits:
        source = hit.get("_source", {})
        embedding_text = source.get("embedding_text", "")
        
        # 권한이 허용된 필드의 실제 값을 추출 (필드 유형 및 텍스트 파싱 처리 포함)
        ...
        # 값이 없거나, 빈 문자열이거나, "미입력"인 경우는 정렬 기준으로 쓸 수 없으므로 다음 문서로 패스
        if value is None or value == "" or value == "미입력":
            continue

        # 1. 인사평가 관련 필드(예: evaluation_grade 등)인 경우
        if field_key.startswith("evaluation"):
            # 등급 문자열(예: "S", "A")을 맵핑 테이블을 통해 비교 가능한 정렬용 점수(정수/실수)로 변환하여 반환
            return EVALUATION_SORT_SCORE.get(str(value).strip())

        # 2. 직급 필드(job_grade)인 경우
        if field_key == "job_grade":
            # 직급명(예: "사원", "대리")을 사전 정의된 정렬용 서열 점수로 변환하여 반환
            return JOB_GRADE_SORT_SCORE.get(str(value).strip())

        # 3. 그 외 일반 필드인 경우 숫자로 변환을 시도 (예: 연봉, 근속연수 등)
        number_value = to_number(value)

        # 숫자로 정상 변환이 가능하다면 해당 숫자 값을 정렬 기준으로 반환
        if number_value is not None:
            return number_value

        # 숫자 변환이 불가능한 일반 텍스트라면 문자열 형태로 정렬 값을 반환
        return str(value)

    # 모든 문서를 돌았으나 유효한 값을 찾지 못한 경우 None을 반환
    return None
