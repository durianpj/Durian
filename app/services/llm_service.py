...
def build_rag_prompt(question: str, context: str) -> str:
    ...
    prompt = f"""
        ...
        변경이력/이력/변경됐어/바뀌었어 등 변화 과정을 묻는 질문은 항상 아래 변경이력 형식을 최우선으로 사용합니다.
        변경이력 질문이면 검색 결과의 "변경이력:" 데이터를 사용합니다.
        변경이력이 있으면 다음 형식으로 답변합니다.
        "OO님의 변경이력은 다음과 같습니다.
        - YYYY-MM-DD HH:MM:SS | 항목 | 이전값 → 변경값"
        변경이력이 없으면 "변경이력이 없습니다."라고 답변합니다.

        부서 질문(현재 부서를 묻는 경우)이면 다음 형식을 사용합니다.
        "OO님의 부서는 OO입니다."

        팀 질문(현재 팀을 묻는 경우)이면 다음 형식을 사용합니다.
        "OO님의 팀은 OO입니다."
